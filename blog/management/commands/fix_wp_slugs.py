# blog/management/commands/fix_wp_slugs.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Found {len(posts_with_invalid_slugs)} posts with invalid slugs"
        )
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Dry run complete. Found {len(posts_with_invalid_slugs)} posts with invalid slugs."
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully fixed {fixed_count} posts with invalid slugs."
                )
            )
