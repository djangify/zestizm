# blog/management/commands/validate_blog.py
    ...
    def handle(self, *args, **options):
        ...
        for category in categories:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"  ✗ Category: {category.name} (slug: {category.slug}) cannot generate URL"
                    )
                )
        ...
        for post in posts:
            if re.search(invalid_slug_pattern, post.slug):
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'  ✗ Post: "{post.title}" has invalid slug: {post.slug}'
                    )
                )
                continue
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'  ✗ Post: "{post.title}" (slug: {post.slug}) cannot generate URL'
                    )
                )
